...
public class ArmyUnit : BaseUnit
{
    ...
    //     // Start recoil without blocking further attacks
    //     if (data.Type == ArmyType.Chicken)
    ...
    //     Debug.Log($"{name}: Step 1 - Dealing damage & playing VFX");
    //     // --- Step 1: Deal damage and play VFX + sound first ---
    //     if (!IsDead() && currentTarget != null && !currentTarget.IsDead())
    //     {
    //         PerformAttackImpact(); // damage + VFX + sound
    //     }

    //     Debug.Log($"{name}: Step 2 - Triggering attack animation");
    //     // --- Step 2: Trigger attack animation while recoiling backwards ---
    //     SetTrigger("Attack");
    ...
    //     float elapsed = 0f;
    //     Debug.Log($"{name}: Step 3 - Starting recoil");
    //     while (elapsed < recoilTime)
    //     {
    //         transform.position = Vector3.Lerp(startPos, endPos, elapsed / recoilTime);
    //         elapsed += Time.deltaTime;
    ...
    //     }
    //     transform.position = endPos;
    //     Debug.Log($"{name}: Step 3 - Recoil complete");

    //     Debug.Log($"{name}: Step 5 - Reached flag, resetting state");
    //     if (agent != null && agent.isActiveAndEnabled && agent.isOnNavMesh)
    //         agent.ResetPath();
    ...
    // private bool IsInGuardingArea()
    // {
    //     // assuming flagPosition is the center of the guarding area and guardRadius defines it
    //     return Vector3.Distance(transform.position, flagPosition) <= guardRadius;
    // }
    ...
    //     // Disable NavMeshAgent to allow manual movement
    //     agent.enabled = false;

    //     // Temporarily disable collider to avoid physics issues
    //     Collider col = GetComponent<Collider>();
    //     if (col != null) col.enabled = false;

    //     // --- Step 1: Deal damage, play VFX & sound ---
    //     PerformAttackImpact();

    //     // --- Step 2: Play attack animation while recoiling ---
    //     SetTrigger("Attack");

    //     // --- Step 3: Recoil backward manually ---
    //     Vector3 startPos = transform.position;
    //     Vector3 endPos = transform.position - transform.forward * recoilDistance;

    //     // Optional: keep on NavMesh surface
    //     UnityEngine.AI.NavMeshHit hit;
    //     if (UnityEngine.AI.NavMesh.SamplePosition(endPos, out hit, 1f, UnityEngine.AI.NavMesh.AllAreas))
    //         endPos = hit.position;
    ...
    //     // --- Step 5: Return to flag ---
    //     targetPosition = guardPosition;
    //     isMoving = true;

    //     // Reset path to avoid weird agent issues
    //     agent.ResetPath();
    //     agent.SetDestination(targetPosition);

    //     // Wait until we reach the flag
    //     while (Vector3.Distance(transform.position, guardPosition) > agent.stoppingDistance + 0.1f)
    //     {
    //         // Optional: smooth rotation toward movement
    //         if (agent.velocity.sqrMagnitude > 0.01f)
    //         {
    //             transform.rotation = Quaternion.Slerp(
    //                 transform.rotation,
    ...
    //     finally
    //     {
    //         // Always reset flags and collider
    //         if (col != null) col.enabled = true;
    //         isRecoiling = false;
    //         canShoot = true;
    //         isMoving = false;
    ...
    //     Vector3 startPos = transform.position;
    //     Vector3 endPos = startPos - transform.forward * recoilDistance;
    ...
    //     // Allow other attacks while returning
    //     isRecoiling = false;

    //     // Return to flag, but don't block attack
    //     agent.enabled = true;
    //     targetPosition = guardPosition;
    //     isMoving = true;
    //     agent.SetDestination(targetPosition);
    ...
    //     // STEP 2: Trigger roll animation
    //     SetTrigger("Attack");

    //     // STEP 3: Smooth recoil backwards
    //     Vector3 startPos = transform.position;
    //     Vector3 endPos = startPos - transform.forward * recoilDistance;
    ...
    //     // STEP 4: Return to flag using NavMesh
    //     yield return StartCoroutine(ReturnToFlagAfterAttack());
    // }
