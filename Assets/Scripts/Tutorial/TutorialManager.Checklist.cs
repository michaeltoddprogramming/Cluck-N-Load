...
public partial class TutorialManager
{
    ...
    {
        { "Farm Basics", new[] { "welcome", "camera_controls", "money_explanation" } },
        { "Building", new[] { "open_build_shop", "build_farmhouse", "build_crop_plot", "build_silo" } },
        { "Planting", new[] { "plant_crop", "harvest_crop" } },
        { "Animals", new[] { "build_chicken_coop", "buy_animals", "feed_animals", "collect_products" } },
        { "Army", new[] { "build_barracks", "recruit_soldiers", "place_flag", "prepare_defense" } }
    };
    ...
    private void ApplyEnhancedToggleStyle(Toggle toggle, bool isCompleted)
    {
        ...
        {
            if (isCompleted)
                LeanTween.color(background.rectTransform, new Color(0.3f, 0.8f, 0.3f, 0.7f), 0.3f);
            else
                background.color = new Color(0.7f, 0.7f, 0.7f, 0.4f);
        }
    }
        ...
        foreach (string stepId in stepIds)
        {
            ...
            if (itemText != null)
            {
                ...
                if (isCompleted)
                {
                    ...
                    LeanTween.value(itemObj, 0f, 1f, 0.5f)
                        .setDelay(0.2f)
                        .setOnUpdate((float val) => {
                            itemText.color = Color.Lerp(Color.white, new Color(0.4f, 1f, 0.4f), val);
                        });
                }
                else
                    itemText.color = Color.white;
    ...
    private void PlayCompletionFeedback()
    {
        ...
        if (checklistPanel != null)
        {
            ...
            LeanTween.scale(checklistPanel, Vector3.one * 1.05f, 0.2f)
            ...
            if (panelImage != null)
            {
                ...
                LeanTween.value(checklistPanel, 0f, 1f, 0.2f)
                    .setOnUpdate((float val) => {
    ...
    {
        CanvasGroup canvasGroup = checklistPanel.GetComponent<CanvasGroup>();
        if (canvasGroup == null)
            canvasGroup = checklistPanel.AddComponent<CanvasGroup>();
            
        LeanTween.cancel(checklistPanel);
        LeanTween.value(checklistPanel, 1f, 0f, 0.3f)
            .setOnUpdate((float val) => { canvasGroup.alpha = val; })
            .setOnComplete(() => {
                UpdateChecklistUI();
                LeanTween.value(checklistPanel, 0f, 1f, 0.3f)
                    .setOnUpdate((float val) => { canvasGroup.alpha = val; });
            });
    }
